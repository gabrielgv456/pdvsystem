...
export const InventoryManagement = () => {
    ...
    const [actualItemEdit, setActualItemEdit] = useState<sharedProdutcsType | undefined>()
    const [ProductsReturnApi, setProductsReturnApi] = useState<sharedProdutcsType[]>([])
    ...
    const inputSearchProductLowwer = normalizeAndLowercase(inputSearchProduct);
    const [dataTransactionsProductsReturnApi, setdataTransactionsProductsReturnApi] = useState<type.TransactionsProductsReturnApi[]>([])
    const ProductsReturnApiFiltered = ProductsReturnApi.filter((product) => normalizeAndLowercase(product.name).includes(inputSearchProductLowwer))
                ...
                <S.Content isDarkMode={Theme.DarkMode}>
                    ...
                    {isLoading ? <CircularProgressSpinner /> :
                        <S.TableListProducts>
                            ...
                            <S.THeadListProducts isDarkMode={Theme.DarkMode}>
                                <tr  >
                                    <S.Th></S.Th>
                                    <S.Th></S.Th>
                                    <S.Th><b>Produto</b></S.Th>
                                    <S.Th><b>Status</b></S.Th>
                                    <S.Th><b>Disponível</b></S.Th>
                                    <S.Th><b>Reservado</b></S.Th>
                                    <S.Th><b>Valor</b></S.Th>
                                    <S.ThCriadoEm ><b>Criado em</b></S.ThCriadoEm>
                                    <S.Th></S.Th>
                                    <S.Th></S.Th>
                                </tr>
                            </S.THeadListProducts>
                            ...
                            <S.TFootListProducts isDarkMode={Theme.DarkMode}>
                                <tr>
                                    <td colSpan={4}>
                                    ...
                                    <td>
                                        <label>Valor Total: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(ProductsReturnApi.reduce((acc, item) => { return acc + (item.totalValue ?? 0) }, 0))}</label>
                                    </td>
                                    <td colSpan={3} style={{ textAlign: 'center', verticalAlign: 'center' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            {PagesExtract > 0 && <span> Página {atualPageExtract + 1} de {PagesExtract}</span>}
